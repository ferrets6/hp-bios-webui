"""
Remote-deploy backend: the container runs on a different machine than the
one with the hp-bioscfg sysfs interface (e.g. a Raspberry Pi controlling a
NAS), so it talks to it over SSH.

Reads need no privilege (the sysfs files are world-readable). Writes and
reboot go through a small root-owned wrapper script on the target machine
(installed by deploy/nas_setup.sh) invoked via a scoped NOPASSWD sudoers
rule, so this container never needs a root password on the target.

Selected automatically by nas_client.py when NAS_HOST is set.
"""
import os
import shlex
import threading
